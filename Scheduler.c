...
int front = 0,rear = -1;
struct process1 *queArr[SIZE] = {0};;
 void enqueue(struct process1 *p)
{
    if(rear == SIZE-1)
    {
        rear = -1;
    ...
    while(finP < num)
    {
        ...
        if( (cp=dequeue()) != NULL)
        {
            ...
            for(int j=0; j<ts; j++)
            {
                ...
                {
                   cp->compTime--;
                    printf("At sec: %d  ",currentT);
                    for(int i=0; i<num; i++)
                    {
                        printf("process %d is %c\t",arrP[i]->id,arrP[i]->state);
                    }
                    printf("\n\n");
                    sleep(1);
                    currentT++;
...
//Shortest Job First - Pre-emptive
void SJCF(processes P[],int n){
	int i,tcurr = 0,b[n],j,x,min_rt, resp[n];
	int sumw=0,sumt=0;
	float avgwt=0.0,avgta=0.0;
    //;;;;;;;;;;;;;;;;
    int remainning = n;
    ...
    FCFS_sort(temp,n);
	temp[0].completion_time = temp[0].running_time + temp[0].arrival_time;
	for(i=0;i<n;i++) b[i] = temp[i].running_time, resp[i] = -1;
	i = -1, j;
	printf("\n GANTT CHART\n\n");
	...
	while (remainning)
    {
		min_rt = (int)1e9;
		for(x=0;x<n;x++){
			if(temp[x].arrival_time <= tcurr && temp[x].flag != 1){
				if(min_rt > b[x]){
					min_rt = b[x];
					j=x;
				}
        ...
        if (min_rt == (int)1e9)     // snap
        {
            tcurr++;
            continue;
        }

		if(i!=j) printf(" %d %s",tcurr,temp[j].name);
		...
		if(b[i]<=0){
			temp[i].flag = 1;
			temp[i].waiting_time = resp[i] - temp[i].arrival_time + 1;
			temp[i].turnarround_time = (tcurr+1) - temp[i].arrival_time;
			sumw+=temp[i].waiting_time;
			sumt+=temp[i].turnarround_time;
...
printf("\n");
    for(i=0;i<num_processes;i++){
            //int z =process_array[i].start_time+process_array[i].burst_time;
           // printf("%d\n",z);
            for(j=process_array[i].start_time;j<process_array[i].burst_time+process_array[i].start_time;j++){
                printf("-");
            }
            printf(" ");
            printf("|");
    ...
    int z;
        for(i=0;i<num_processes;i++){
            ...
            for(z=process_array[i].start_time;z<(process_array[i].burst_time+process_array[i].start_time)-1;z++){
                printf(" ");
            }
